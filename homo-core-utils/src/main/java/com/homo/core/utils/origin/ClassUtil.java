...
import com.homo.core.utils.origin.single.SingletonPrototype;
...
public class ClassUtil {
	...
	public static Set<String> getClassPaths(String packageName, boolean isDecode) {
		...
		try {
			...
			throw new UtilException("Loading classPath ["+packagePath+"] error!" ,e);
		}
		final Set<String> paths = new HashSet<String>();
		String path;
		while (resources.hasMoreElements()) {
			path = resources.nextElement().getPath();
			paths.add(isDecode ? URLUtil.decode(path, Charset.defaultCharset()) : path);
		}
		return paths;
	}
	...
	public static <T> T invoke(String className, String methodName, boolean isSingleton, Object[] args) {
		...
		try {
			...
			if (isStatic(method)) {
				...
				return ReflectUtil.invoke(isSingleton ? SingletonPrototype.get(clazz) : clazz.newInstance(), method, args);
			}
		} catch (Exception e) {
