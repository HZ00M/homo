...
public class RandomUtil {
	 ...
	 *
	 */
	public static LocalDateTime randomDay(int min, int max) {

		return randomDate(LocalDateTime.now(), ChronoField.DAY_OF_YEAR , min, max);
	}
	 ...
	 *
	 * @param baseDate 基准日期
	 * @param field 偏移的时间字段，例如时、分、秒等
	 * @param min 偏移最小量，可以为负数表示过去的时间（包含）
	 * @param max 偏移最大量，可以为负数表示过去的时间（不包含）
	 * @return 随机日期
	 *
	 */
	public static LocalDateTime randomDate(LocalDateTime baseDate, TemporalField field, int min, int max) {
		if (null == baseDate) {
			baseDate = LocalDateTime.now();
		}

		return baseDate.with(field, randomInt(min, max));
	}
