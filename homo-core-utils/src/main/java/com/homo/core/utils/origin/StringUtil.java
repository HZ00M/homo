...
import com.homo.core.utils.origin.text.StrFormatter;
import com.homo.core.utils.origin.version.VersionSimpleComparator;
...
/**
 * 字符串工具类
 * 参考apache common StringUtils 添加了很多自己的方法
 *
 *
 */
public final class StringUtil {
	...
	public static String subPreGbk(CharSequence str, int len, CharSequence suffix) {
		...
		b = str.toString().getBytes(CHARSET_GBK);
		if (b.length <= len) {
			return str.toString();
		}
		for (int i = 0; i < len; i++) {
		...
		return new String(b, 0, len, CHARSET_GBK) + suffix;
	}
	...
	public static int compareVersion(CharSequence version1, CharSequence version2) {
		return VersionSimpleComparator.INSTANCE.compare(str(version1), str(version2));
	}
	 ...
	 * @return 替换后的字符串
	 * @see RegexUtil#replaceAll(CharSequence, Pattern, Function)
	 *
	 */
	public static String replace(CharSequence str, Pattern pattern, Function<Matcher, String> replaceFun) {
		return RegexUtil.replaceAll(str, pattern, replaceFun);
	}
	 ...
	 * @param replaceFun 决定如何替换的函数
	 * @return 替换后的字符串
	 * @see RegexUtil#replaceAll(CharSequence, String, Function)
	 *
	 */
	public static String replace(CharSequence str, String regex, Function<Matcher, String> replaceFun) {
		return RegexUtil.replaceAll(str, regex, replaceFun);
	}
	 ...
	 *
	 */
	public static class StrSpliter {
