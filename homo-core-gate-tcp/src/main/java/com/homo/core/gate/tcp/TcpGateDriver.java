...
import com.homo.core.configurable.gate.GateProperties;
...
import com.homo.core.gate.tcp.handler.TcpHandler;
...
public class TcpGateDriver<T> implements GateDriver<T>, DriverModule {
    ...
    private AttributeKey<GateClient> clientKey = AttributeKey.valueOf("user");
    ...
    private Tuple3<List<ChannelHandler>, List<TcpHandler>, List<ChannelHandler>> customHandlers = Tuples.of(new ArrayList<>(), new ArrayList<>(), new ArrayList<>());

    public void init() {
        if (Epoll.isAvailable()) {
            bossGroup = new EpollEventLoopGroup(gateProperties.bossNum, bossExecutor);
            workGroup = new EpollEventLoopGroup(gateProperties.workNum);
            log.info("TcpGateDriver initialize using epoll");
        } else {
            bossGroup = new NioEventLoopGroup(gateProperties.bossNum, bossExecutor);
            workGroup = new NioEventLoopGroup(gateProperties.workNum);
    ...
    public void startGate(GateServer<T> gateServer) {
        try {
            ...
            handlers.addAll(customHandlers.getT1());//定制前处理器
            handlers.addAll(customHandlers.getT2());//用户定制处理器
            handlers.addAll(customHandlers.getT3());//定制后处理器
            ChannelFuture channelFuture = new ServerBootstrap()//创建ServerBootstrap实例
                    ...
                    .childHandler(new ServerChanelInitializer(handlers))//在ServerChannelInitializer中初始化ChannelPipeline责任链，并添加到serverBootstrap中
            ...
            if (channelFuture.isSuccess()) {
                log.info("TcpGateDriver startGateServer success listener {}", gateServer.getPort());
                serverChannel = channelFuture.channel();
                isRunning = true;
            }

        } catch (Exception e) {
            log.error("TcpGateDriver startGateServer fail", e);
            System.exit(-1);
        }
    }
    ...
    @Override
    public Homo pong(GateClient<T> gateClient,  T msg) {
    ...
    public <T> void registerPostHandler(TcpHandler<T> handler) {
