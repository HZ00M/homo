...
public class EntityRpcProxy implements MethodInterceptor {
    ...
    private Homo rpcCall(String type, String id, String methodName, EntityRequest entityRequest) throws Exception {
        ...
        return GetBeanUtil.getBean(ServiceStateMgr.class).getServiceInfo(type)
                .nextDo(serviceInfo -> {
                    if (serviceInfo == null) {
                    ...
                    return ExchangeHostName.exchange(serviceInfo, entityRequest)
                            .nextDo(realName -> {
                                ...
                                return rpcClientMgr.getAgentClient(realName, serviceInfo)
                                        ...
                                        .nextDo(ret -> {
                                            Object entityData = serviceEntityRpcInfo.unSerializeReturnValue(IEntityService.default_entity_call_method, rpcContent);
                                            ...
                                            logicContent.setReturn(data[0]);
                                            Object dataObjs = rpcHandlerInfoForClientMap.get(interfaceType).unSerializeReturnValue(methodName, logicContent);
                                            return Homo.result(dataObjs);
                                        });
                            });
                });
    }
